...
def test_cli_help(runner):
    ...
    assert "RetroSpark CLI" in result.output
...
def test_cli_init(runner, mocker, tmp_path):
    ...
    mocker.patch("retrospark.cli.get_brain_dir", return_value=tmp_path / "brain")
    ...
    assert output_json["remote_url"] == "git@github.com:test.git"
...
def test_cli_sync_no_init(runner, mocker, tmp_path):
    ...
    mocker.patch("retrospark.cli.get_brain_dir", return_value=tmp_path / "nonexistent")
    ...
    assert "brain directory not initialized" in output_json["message"].lower()

def test_cli_sync_success(runner, mocker, tmp_path):
    brain_dir = tmp_path / "brain"
    brain_dir.mkdir(parents=True)
    mocker.patch("retrospark.cli.get_brain_dir", return_value=brain_dir)
