---
name: retrospark
description: Saves your current conversation state, thoughts, and extracted code as structured Markdown notes into the user's local digital second brain (and automatically versions it with Git).
---

# RetroSpark: Personal Digital Second Brain Exporter
...
1. **Invoke the Tracker:**
   ...
   *Support sources include: `antigravity`, `claude`, `codex`, `gemini`, `opencode`, `openclaw`, `all`.*
...
2. **Read the JSON Response:**
   ...
   - If `status == "error"`, read the `message` field and inform the user. It may say the brain directory is missing, asking you to run `retrospark init --json` first.
